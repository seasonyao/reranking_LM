...
rerank_transformer_dict = {}
for key in model_dict.keys():
    if key.startswith('transformer'):
        rerank_transformer_dict['rerank_transformer'+key.split('transformer')[1]] = model_dict[key]
model_dict.update(rerank_transformer_dict)

model = rerankGPT2LMHeadModel_not_sharing_stage1_stage2.from_pretrained("gpt2", config=configuration, state_dict = model_dict,
                                                                        MAX_LEN = MAX_LEN,
                                                                        CAN_NUM = CAN_NUM, 
